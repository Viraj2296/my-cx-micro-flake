...
import (
	...
	"cx-micro-flake/services/fixed_assets/handler"
	"fmt"
	"go.uber.org/zap"
	"os"
...
func InitStandaloneService(router *gin.Engine, projectConfig []common.ProjectDatasourceConfig) error {
	...
	err := baseService.Init("../services/fixed_assets/conf/config.json", projectConfig)
	if err != nil {
		fmt.Printf("failed to initialize fixed assets service [%s] due to error [%s]", "error", err.Error())
	}
	...
	config.Load(file.NewSource(
		file.WithPath("../services/fixed_assets/conf/config.json"),
	))
	...
	var qrCodeDomainUrl string
	if err := config.Get("qrDomainUrl").Scan(&qrCodeDomainUrl); err != nil {
		baseService.Logger.Error("unable to read the QR code domain", zap.String("error", err.Error()))
		os.Exit(1)
	}
	...
	fixedAssetService := handler.FixedAssetService{
		BaseService:             baseService,
		ComponentContentConfig:  contentConfig,
		QRCodeDomainUrl:         qrCodeDomainUrl,
		EmailNotificationDomain: emailNotificationDomain,
	}
	fixedAssetService.InitRouter(router)

	var fixedAssetsInterface common.FixedAssetsInterface
	fixedAssetsInterface = &fixedAssetService

	service := component.ServiceConfig{
		ServiceType:      "fixed_assets",
		ServiceName:      "fixed_assets_module",
		ServiceInterface: fixedAssetsInterface,
	}
