...
func (v *FixedAssetService) getOverview(ctx *gin.Context) {
	var overviewResponse = make([]component.OverviewResponse, 0)
	projectId := ctx.Param("projectId")
	dbConnection := v.BaseService.ServiceDatabases[projectId]

	var err error
	listOfAssets, err := GetObjects(dbConnection, FixedAssetMasterTable)
	if err != nil {
	...
	var noOfAssets = len(*listOfAssets)
	overviewData := make([]component.OverviewData, 0)
	totalAssets := getKPIData(noOfAssets, "Total Assets")

	overviewData = append(overviewData, totalAssets)
