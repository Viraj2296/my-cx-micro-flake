...
func (v *FixedAssetService) emailGenerator(dbConnection *gorm.DB, templateId int, user int, primaryComponent string, primaryObjectId int) error {

	condition := " object_info ->>'$.templateType'= " + strconv.Itoa(templateId) + " AND object_info ->>'$.isTemplateEnabled' = 'true'"
	listOfObjects, err := GetConditionalObjects(dbConnection, FixedAssetEmailTemplateTable, condition)
	...
	if err == nil {
		...
		err, commonObject := Get(dbConnection, FixedAssetEmailTemplateFieldTable, templateFieldId)
		...
		v.BaseService.Logger.Info("generated email template", zap.String("template", emailTemplate))
