...
func (v *EmailHandler) IsEmailTemplateExist(dbConnection *gorm.DB, templateType int) bool {
	...
	listOfObjects, err := database.GetConditionalObjects(dbConnection, const_util.ITServiceEmailTemplateTable, condition)
	if err == nil {
...
func (v *EmailHandler) EmailGeneratorByEmail(dbConnection *gorm.DB, templateId int, targetEmailId string, primaryComponent string, primaryObjectId int) error {
	...
	if err == nil {
		...
		err, commonObject := database.Get(dbConnection, const_util.ITServiceEmailTemplateFieldTable, templateFieldId)
