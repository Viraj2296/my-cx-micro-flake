...
func (v *ITService) CreateUserRecordMessage(projectId string, componentName, message string, recordId int, userId int, existingData *component.GeneralObject, updatedData *component.GeneralObject) error {
	...
	recordTrail := database.ITServiceRecordTrail{
		RecordId:      recordId,
...
func (v *ITService) getComponentRecordTrails(ctx *gin.Context) {
	...
	listOfObjects, err := database.GetConditionalObjectsOrderBy(dbConnection, const_util.ITServiceRecordTrailTable, conditionString, orderByCondition)
	if err != nil {
...
func (v *ITService) getSourceObjectVersions(ctx *gin.Context) {
	projectId := util.GetProjectId(ctx)
	recordId := util.GetRecordId(ctx)
...
func (v *ITService) handleRollBack(ctx *gin.Context) {
	projectId := util.GetProjectId(ctx)
	recordId := util.GetRecordId(ctx)
	dbConnection := v.BaseService.ServiceDatabases[projectId]
	componentName := ctx.Param("componentName")
	...
	listOfObjects, err := database.GetConditionalObjects(dbConnection, const_util.ITServiceRecordTrailTable, conditionString)
