...
func (v *ActionService) HandleSapApprove(ctx *gin.Context) {

	recordId, basicUserInfo, serviceRequestInfo := v.getBasicInfo(ctx)
	if util.InterfaceToBool(serviceRequestInfo["canSapApprove"]) {
		...
		existingActionRemarks = append(existingActionRemarks, database.ActionRemarks{
			...
			Status:        "APPROVED BY SAP MANAGER",
			UserId:        basicUserInfo.UserId,
			Remarks:       "Great, your request has been approved by Sap Manager. Please wait until IT is approving your request",
		...
		for _, workflowUser := range listOfWorkflowUsers {
			err := v.EmailHandler.EmailGenerator(v.Database, const_util.SapApproveEmailTemplateType, workflowUser, const_util.ITServiceMySapRequestComponent, recordId)
			if err != nil {
				v.Logger.Error("handle SAP approve has failed due to sending email to workflow users", zap.String("error", err.Error()))
			}
		}
