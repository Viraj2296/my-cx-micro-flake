...
func (v *NotificationService) CreateActionRecordMessage(projectId string, componentName, message string, recordId int, userId int, actionFields map[string]interface{}) error {
	...
	recordTrail := NotificationRecordTrail{
		RecordId:      recordId,
...
func (v *NotificationService) CreateUserRecordMessage(projectId string, componentName, message string, recordId int, userId int, existingData *component.GeneralObject, updatedData *component.GeneralObject) error {
	//TODO change this to int64  as we are using recordId (int) datatype, not a string
	resourceInfo := common.ResourceInfo{}
	resourceMeta := common.ResourceMeta{}
...
func (v *NotificationService) getComponentRecordTrails(ctx *gin.Context) {
	...
	listOfObjects, err := GetConditionalObjectsOrderBy(dbConnection, NotificationRecordTrailTable, conditionString, orderByCondition)
	if err != nil {
