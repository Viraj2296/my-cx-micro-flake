...
func (v *MaintenanceService) CreateUserRecordMessage(projectId string, componentName, message string, recordId int, userId int, existingData *component.GeneralObject, updatedData *component.GeneralObject) error {
	...
	recordTrail := MaintenanceRecordTrail{
		RecordId:      recordId,
...
func (v *MaintenanceService) getComponentRecordTrails(ctx *gin.Context) {
	...
	listOfObjects, err := GetConditionalObjectsOrderBy(dbConnection, MaintenanceRecordTrailTable, conditionString, orderByCondition)
	if err != nil {
	...
	listOfRecordSchema := v.ComponentManager.GetRecordSchema(componentName)
	response := common.GetRecordTrailResponse_V1(zone, dbConnection, listOfObjects, listOfRecordSchema)
