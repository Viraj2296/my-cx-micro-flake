...
func (v *MaintenanceService) GetComponents() []datatypes.JSON {
	var arrayOfObject []datatypes.JSON
	dbConnection := v.BaseService.ServiceDatabases[ProjectID]
	listOfObjects, err := GetObjects(dbConnection, MaintenanceComponentTable)
	if err == nil {
...
func (v *MaintenanceService) UpdateComponent(componentName, targetTable string, serialisedObject datatypes.JSON) error {
	var conditionQuery = " object_info->>'$.componentName' = '" + componentName + "' AND object_info->'$.targetTable' = '" + targetTable + "'"
	dbConnection := v.BaseService.ServiceDatabases[ProjectID]
	listOfObjects, err := GetConditionalObjects(dbConnection, MaintenanceComponentTable, conditionQuery)
	if err == nil {
		...
		err := Update(dbConnection, MaintenanceComponentTable, (*listOfObjects)[0].Id, updatingData)
...
func (v *MaintenanceService) CreateComponent(serialisedObject datatypes.JSON) (int, error) {
	dbConnection := v.BaseService.ServiceDatabases[ProjectID]
	generalObject := component.GeneralObject{ObjectInfo: serialisedObject}
	err, recordId := Create(dbConnection, MaintenanceComponentTable, generalObject)
	if err == nil {
		v.LoadInitComponents()
	}
	return recordId, err
