...
func (v *TraceabilityService) getTraceabilityOverview(ctx *gin.Context) {
	var overviewResponse = make([]component.OverviewResponse, 0)
	dbConnection := v.BaseService.ReferenceDatabase

	var err error
	listOfQABatchRecords, err := GetObjects(dbConnection, TraceabilityOrdersTable)
	if err != nil {
	...
	var totalBatchRecords int
	totalBatchRecords = len(*listOfQABatchRecords)

	overviewData := make([]component.OverviewData, 0)
	totalBatchRecordsKPI := getKPIData(totalBatchRecords, "Batch Orders")
	overviewData = append(overviewData, totalBatchRecordsKPI)
