...
func (v *TraceabilityService) CreateUserRecordMessage(projectId string, componentName, message string, recordId int, userId int, existingData *component.GeneralObject, updatedData *component.GeneralObject) error {
	...
	recordTrail := TraceabilityRecordTrail{
		RecordId:      recordId,
...
func (v *TraceabilityService) getComponentRecordTrails(ctx *gin.Context) {
	...
	listOfObjects, err := GetConditionalObjectsOrderBy(dbConnection, TraceabilityRecordTrailTable, conditionString, orderByCondition)
	if err != nil {
