...
func (v *ContentService) getOverview(ctx *gin.Context) {
	...
	for _, generalInterface := range *listOfContent {
		contentMaster := ContentMaster{ObjectInfo: generalInterface.ObjectInfo}
		if contentMaster.GetContentInfo().IsFile {
			numberOfFiles += 1
		} else {
			numberOfFolders += 1
		}
	...
	totalFolder := getKPIData(numberOfFolders, "Total Folders")
	totalFiles := getKPIData(numberOfFiles, "Total Files")

	overviewData = append(overviewData, totalFolder)
	overviewData = append(overviewData, totalFiles)
