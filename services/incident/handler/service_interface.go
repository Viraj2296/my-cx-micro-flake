...
func (v *IncidentService) GetComponents() []datatypes.JSON {
	var arrayOfObject []datatypes.JSON
	dbConnection := v.BaseService.ServiceDatabases[ProjectID]
	listOfObjects, err := GetObjects(dbConnection, IncidentComponentTable)
	if err == nil {
...
func (v *IncidentService) UpdateComponent(componentName, targetTable string, serialisedObject datatypes.JSON) error {
	var conditionQuery = " object_info->>'$.componentName' = '" + componentName + "' AND object_info->'$.targetTable' = '" + targetTable + "'"
	dbConnection := v.BaseService.ServiceDatabases[ProjectID]
	listOfObjects, err := GetConditionalObjects(dbConnection, IncidentComponentTable, conditionQuery)
	if err == nil {
		...
		err := Update(dbConnection, IncidentComponentTable, (*listOfObjects)[0].Id, updatingData)
...
func (v *IncidentService) CreateComponent(serialisedObject datatypes.JSON) (int, error) {
	dbConnection := v.BaseService.ServiceDatabases[ProjectID]
	generalObject := component.GeneralObject{ObjectInfo: serialisedObject}
	err, recordId := Create(dbConnection, IncidentComponentTable, generalObject)
	if err == nil {
		v.LoadInitComponents()
	}
	return recordId, err
