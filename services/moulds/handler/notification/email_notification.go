...
func (v *EmailHandler) isEmailTemplateExist(dbConnection *gorm.DB, templateType int) bool {
	...
	listOfObjects, err := database.GetConditionalObjects(dbConnection, const_util.MouldEmailTemplateTable, condition)
	if err == nil {
...
func (v *EmailHandler) EmailGenerator(dbConnection *gorm.DB, templateId int, user int, primaryComponent string, primaryObjectId int) error {
	...
	if err == nil {
		...
		err, commonObject := database.Get(dbConnection, const_util.MouldEmailTemplateFieldTable, emailTemplateInfo.TemplateType)
		...
		err = notificationService.CreateMessages(const_util.ProjectID, emailMessages)
		if err != nil {
			v.Logger.Error("errro creating notification messages", zap.String("error", err.Error()))
		}
	} else {
