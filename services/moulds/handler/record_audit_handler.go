...
func (v *MouldService) CreateUserRecordMessage(projectId string, componentName, message string, recordId int, userId int, existingData *component.GeneralObject, updatedData *component.GeneralObject) error {
	...
	recordTrail := database.MouldsRecordTrail{
		RecordId:      recordId,
...
func (v *MouldService) getComponentRecordTrails(ctx *gin.Context) {
	...
	listOfObjects, err := database.GetConditionalObjectsOrderBy(dbConnection, const_util.MouldsRecordTrailTable, conditionString, orderByCondition)
	if err != nil {
