...
import (
	...
	"cx-micro-flake/services/moulds/handler"
	"fmt"
	"os"
...
func InitStandaloneService(router *gin.Engine, projectConfig []common.ProjectDatasourceConfig) error {
	...
	err := baseService.Init("../services/moulds/conf/config.json", projectConfig)
	if err != nil {
		fmt.Printf("failed to initialize service [%s] due to error [%s]", "machine", err.Error())
	}
	baseService.Logger.Info("creating mould service")

	//we don't get any error here as we already loaded the config without any errors
	config.Load(file.NewSource(
		file.WithPath("../services/moulds/conf/config.json"),
	))
	...
	mouldService := handler.MouldService{
		BaseService:             baseService,
		ComponentContentConfig:  contentConfig,
		EmailNotificationDomain: emailNotificationDomain,
	}
	mouldService.InitRouter(router)
	var lifeNotificationPoolingInterval int
	if err := config.Get("lifeNotificationPoolingInterval").Scan(&lifeNotificationPoolingInterval); err != nil {
		baseService.Logger.Error("unable to read the lifeNotificationPoolingInterval", zap.String("error", err.Error()))
		os.Exit(1)
	}

	var mouldServiceInterface common.MouldInterface
	mouldServiceInterface = &mouldService

	service := component.ServiceConfig{
		ServiceType:      "moulds",
		ServiceName:      "moulds_module",
		ServiceInterface: mouldServiceInterface,
	}
