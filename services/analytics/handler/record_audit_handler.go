...
func (as *AnalyticsService) CreateUserRecordMessage(projectId string, componentName, message string, recordId int, userId int, existingData *component.GeneralObject, updatedData *component.GeneralObject) error {
	...
	recordTrail := AnalyticsRecordTrail{
		RecordId:      recordId,
	...
	as.BaseService.Logger.Info("creating a record trail id", zap.Int("trail_id", recordId))
	return err
}
...
func (as *AnalyticsService) getComponentRecordTrails(ctx *gin.Context) {
	...
	listOfObjects, err := GetConditionalObjectsOrderBy(dbConnection, AnalyticsRecordTrailTable, conditionString, orderByCondition)
	if err != nil {
