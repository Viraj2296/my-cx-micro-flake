...
import (
	...
	"cx-micro-flake/services/manufacturing/handler"
	"fmt"
	"go.uber.org/zap"
	"os"
...
func InitStandaloneService(router *gin.Engine, projectConfig []common.ProjectDatasourceConfig) error {
	...
	err := baseService.Init("../services/manufacturing/conf/config.json", projectConfig)
	if err != nil {
		fmt.Printf("failed to initialize service [%s] due to error [%s]", "scheduler", err.Error())
	}
	baseService.Logger.Info("creating manufacturing service")

	//we don't get any error here as we already loaded the config without any errors
	config.Load(file.NewSource(
		file.WithPath("../services/manufacturing/conf/config.json"),
	))
	...
	manufacturingService := handler.ManufacturingService{
		BaseService:            baseService,
		ComponentContentConfig: contentConfig,
	}
	manufacturingService.InitRouter(router)

	var manufacturingInterface common.ManufacturingInterface
	manufacturingInterface = &manufacturingService

	service := component.ServiceConfig{
		ServiceType:      "manufacturing",
		ServiceName:      "manufacturing_module",
		ServiceInterface: manufacturingInterface,
	}
