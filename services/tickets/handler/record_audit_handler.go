...
func (v *TicketsService) CreateUserRecordMessage(projectId string, componentName, message string, recordId int, userId int, existingData *component.GeneralObject, updatedData *component.GeneralObject) error {
	...
	recordTrail := TicketsServiceRecordTrail{
		RecordId:      recordId,
...
func (ts *TicketsService) getComponentRecordTrails(ctx *gin.Context) {
	...
	listOfObjects, err := GetConditionalObjectsOrderBy(dbConnection, TicketsServiceRecordTrailTable, conditionString, orderByCondition)
	if err != nil {
		ts.BaseService.Logger.Error("error loading resources", zap.String("error", err.Error()))
