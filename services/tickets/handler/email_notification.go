...
func (ts *TicketsService) isEmailTemplateExist(dbConnection *gorm.DB, templateType int) bool {
	...
	listOfObjects, err := GetConditionalObjects(dbConnection, TicketsServiceEmailTemplateTable, condition)
	if err == nil {
...
func (ts *TicketsService) emailGenerator(dbConnection *gorm.DB, templateId int, user int, primaryComponent string, primaryObjectId int) error {
	...
	if err == nil {
		...
		err, commonObject := Get(dbConnection, TicketsServiceEmailTemplateFieldTable, emailTemplateInfo.TemplateType)

		targetTable := ts.ComponentManager.GetTargetTable(primaryComponent)
		...
		if err == nil {
						...
						if index != -1 {
							...
							if moduleField.ComponentName == primaryComponent {
								...
								if moduleField.Type == "href" {
									var objectLink = ts.EmailNotificationDomain + "?routeLink=" + moduleField.MenuId + "&recordId=" + strconv.Itoa(primaryObjectId)
								...
								if moduleField.IsObjectField {
									...
									targetTable = ts.ComponentManager.GetTargetTable(moduleField.ComponentName)
									...
									if err == nil {
										...
										if moduleField.Type == "href" {
											var objectLink = ts.EmailNotificationDomain + "?routeLink=" + moduleField.MenuId + "&recordId=" + strconv.Itoa((*listOfObjects)[0].Id)
											emailTemplate = strings.Replace(emailTemplate, moduleField.Display, objectLink, -1)
										} else {
			...
			ts.BaseService.Logger.Error("error getting template field", zap.String("error", err.Error()))
			return err
		}
		emailTemplate = util.FormatStringHTML(emailTemplate)
		ts.BaseService.Logger.Info("generated email template", zap.Any("template", emailTemplate))
		...
		ts.BaseService.Logger.Error("error getting email template", zap.String("error", err.Error()))
	}
	return err

}

func (ts *TicketsService) isWorkflowEmailExist(dbConnection *gorm.DB, tableName string, budgetWorkflowId int) []int {
...
func (ts *TicketsService) notifyWorkflowEmail(dbConnection *gorm.DB, userList []int, componentName string, templateId int, id int) {

	for _, user := range userList {
		ts.emailGenerator(dbConnection, templateId, user, componentName, id)
	}
