...
func (v *SchedulerService) CreateActionRecordMessage(projectId string, componentName, message string, recordId int, userId int, actionFields map[string]interface{}) error {
	...
	recordTrail := SchedulerRecordTrail{
		RecordId:      recordId,
...
func (v *SchedulerService) CreateUserRecordMessage(projectId string, componentName, message string, recordId int, userId int, existingData *component.GeneralObject, updatedData *component.GeneralObject) error {
	//TODO change this to int64  as we are using recordId (int) datatype, not a string
	resourceInfo := common.ResourceInfo{}
	resourceMeta := common.ResourceMeta{}
...
func (v *SchedulerService) getComponentRecordTrails(ctx *gin.Context) {
	...
	listOfObjects, err := GetConditionalObjectsOrderBy(dbConnection, const_util.SchedulerRecordTrailTable, conditionString, orderByCondition)
	if err != nil {
