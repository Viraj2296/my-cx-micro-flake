...
func (v *WorkHourAssignmentService) getSummaryResponse(ctx *gin.Context, projectId string) {
	...
	if isHead {
		...
		listOfObjects, err = GetObjects(dbConnection, WorkHourAssignmentRequestTable)
		...
		listOfObjects, err = GetConditionalObjects(dbConnection, WorkHourAssignmentRequestTable, userBasedQuery)
