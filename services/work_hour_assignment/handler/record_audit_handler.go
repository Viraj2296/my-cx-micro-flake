...
func (v *WorkHourAssignmentService) CreateUserRecordMessage(projectId string, componentName, message string, recordId int, userId int, existingData *component.GeneralObject, updatedData *component.GeneralObject) error {
	...
	recordTrail := WorkHourAssignmentRecordTrail{
		RecordId:      recordId,
...
func (v *WorkHourAssignmentService) getComponentRecordTrails(ctx *gin.Context) {
	...
	listOfObjects, err := GetConditionalObjectsOrderBy(dbConnection, WorkHourAssignmentRecordTrailTable, conditionString, orderByCondition)
	if err != nil {
