...
func (v *WorkHourAssignmentService) GetComponents() []datatypes.JSON {
	var arrayOfObject []datatypes.JSON
	dbConnection := v.BaseService.ServiceDatabases[ProjectID]
	listOfObjects, err := GetObjects(dbConnection, WorkHourAssignmentComponentTable)
	if err == nil {
...
func (v *WorkHourAssignmentService) UpdateComponent(componentName, targetTable string, serialisedObject datatypes.JSON) error {
	var conditionQuery = " object_info->>'$.componentName' = '" + componentName + "' AND object_info->'$.targetTable' = '" + targetTable + "'"
	dbConnection := v.BaseService.ServiceDatabases[ProjectID]
	listOfObjects, err := GetConditionalObjects(dbConnection, WorkHourAssignmentComponentTable, conditionQuery)
	if err == nil {
		...
		err := Update(dbConnection, WorkHourAssignmentComponentTable, (*listOfObjects)[0].Id, updatingData)
...
func (v *WorkHourAssignmentService) CreateComponent(serialisedObject datatypes.JSON) (int, error) {
	dbConnection := v.BaseService.ServiceDatabases[ProjectID]
	generalObject := component.GeneralObject{ObjectInfo: serialisedObject}
	err, recordId := Create(dbConnection, WorkHourAssignmentComponentTable, generalObject)
	if err == nil {
		v.LoadInitComponents()
	}
	return recordId, err
