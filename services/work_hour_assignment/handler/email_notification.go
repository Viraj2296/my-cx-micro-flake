...
func (v *WorkHourAssignmentService) isEmailTemplateExist(dbConnection *gorm.DB, templateType int) bool {
	...
	listOfObjects, err := GetConditionalObjects(dbConnection, WorkHourAssignmentEmailTemplateTable, condition)
	if err == nil {
...
func (v *WorkHourAssignmentService) emailGenerator(dbConnection *gorm.DB, templateId int, user int, primaryComponent string, primaryObjectId int) error {

	condition := " object_info ->>'$.templateType'= " + strconv.Itoa(templateId) + " AND object_info ->>'$.isTemplateEnabled' = 'true'"
	listOfObjects, err := GetConditionalObjects(dbConnection, WorkHourAssignmentEmailTemplateTable, condition)
	v.BaseService.Logger.Info("generated email templates", zap.Any("template", listOfObjects))
	if err == nil {
		...
		err, commonObject := Get(dbConnection, WorkHourAssignmentEmailTemplateFieldTable, emailTemplateInfo.TemplateType)
		v.BaseService.Logger.Info("generated email template field", zap.Any("fields", commonObject))
		...
		v.BaseService.Logger.Info("object fields", zap.Any("primaryObjectFields", primaryObjectFields))
		...
		v.BaseService.Logger.Info("generated email template", zap.Any("template", emailTemplate))
...
func (v *WorkHourAssignmentService) isWorkflowEmailExist(dbConnection *gorm.DB, tableName string, budgetWorkflowId int) []int {
...
func (v *WorkHourAssignmentService) notifyWorkflowEmail(dbConnection *gorm.DB, userList []int, componentName string, templateId int, id int) {
