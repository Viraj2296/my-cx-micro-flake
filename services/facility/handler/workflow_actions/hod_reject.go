...
func (v *ActionService) HandleDepartmentHeadReject(ctx *gin.Context) {
	...
	if util.InterfaceToBool(serviceRequestInfo["canHODReject"]) {
		...
		v.EmailHandler.EmailGenerator(dbConnection, const_util.HeadOfDepartmentRejectEmailTemplateType, util.InterfaceToInt(serviceRequestInfo["createdBy"]), const_util.FacilityServiceMyDepartmentRequestComponent, recordId)
