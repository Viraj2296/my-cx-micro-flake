...
func (v *ActionService) HandleDepartmentHeadReturn(ctx *gin.Context) {
	...
	if util.InterfaceToBool(serviceRequestInfo["canHODReturn"]) {
		if !v.EmailHandler.IsEmailTemplateExist(dbConnection, const_util.HeadOfDepartmentReturnEmailTemplateType) {
		...
		v.EmailHandler.EmailGenerator(dbConnection, const_util.HeadOfDepartmentReturnEmailTemplateType, util.InterfaceToInt(serviceRequestInfo["createdBy"]), const_util.FacilityServiceMyRequestComponent, recordId)
