...
func (v *FacilityService) getOverview(ctx *gin.Context) {
	var overviewResponse = make([]component.OverviewResponse, 0)
	projectId := ctx.Param("projectId")
	dbConnection := v.BaseService.ServiceDatabases[projectId]

	var err error
	listOfRequests, err := database.GetObjects(dbConnection, const_util.FacilityServiceRequestTable)
	if err != nil {
	...
	user.BackgroundColor = getBackgroundColour(dbConnection, 1, const_util.FacilityServiceRequestStatusTable)
	overviewData = append(overviewData, user)
	hod.BackgroundColor = getBackgroundColour(dbConnection, 2, const_util.FacilityServiceRequestStatusTable)
	overviewData = append(overviewData, hod)
	reviewParty.BackgroundColor = getBackgroundColour(dbConnection, 3, const_util.FacilityServiceRequestStatusTable)
	overviewData = append(overviewData, reviewParty)
	executionParty.BackgroundColor = getBackgroundColour(dbConnection, 4, const_util.FacilityServiceRequestStatusTable)
