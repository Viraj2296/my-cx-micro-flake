...
func (v *LabourManagementService) CreateUserRecordMessage(projectId string, componentName, message string, recordId int, userId int, existingData *component.GeneralObject, updatedData *component.GeneralObject) error {
	...
	recordTrail := database.LabourManagementRecordTrail{
		RecordId:      recordId,
...
func (v *LabourManagementService) getComponentRecordTrails(ctx *gin.Context) {
	...
	err, listOfObjects := database.GetConditionalObjectsOrderBy(dbConnection, const_util.LabourManagementRecordTrailTable, conditionString, orderByCondition)
	if err != nil {
	...
	response := common.GetRecordTrailResponseV1(zone, listOfObjects)
