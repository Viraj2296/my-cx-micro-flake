...
func CreateShiftProductionAttendanceLink(database *gorm.DB, objectInterface LabourManagementShiftProductionAttendance) error {
...
func CreateRecordTrail(database *gorm.DB, objectInterface LabourManagementRecordTrail) (error, int) {
	err := database.Create(&objectInterface).Error
...
func ArchiveObject(database *gorm.DB, table string, objectInterface component.GeneralObject) error {
	...
	serialisedData, err := json.Marshal(objectFields)
	if err == nil {
		updatingData["object_info"] = serialisedData
		err = database.Table(table).Take(&existingObject).UpdateColumns(updatingData).Error
	}
