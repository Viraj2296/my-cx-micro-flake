...
func (v *ProductionOrderService) CreateActionRecordMessage(projectId string, componentName, message string, recordId int, userId int, actionFields map[string]interface{}) error {
	...
	recordTrail := ProductionOrderRecordTrail{
		RecordId:      recordId,
...
func (v *ProductionOrderService) CreateUserRecordMessage(projectId string, componentName, message string, recordId int, userId int, existingData *component.GeneralObject, updatedData *component.GeneralObject) error {
	//TODO change this to int64  as we are using recordId (int) datatype, not a string
	resourceInfo := common.ResourceInfo{}
	resourceMeta := common.ResourceMeta{}
...
func (v *ProductionOrderService) getComponentRecordTrails(ctx *gin.Context) {
	...
	listOfObjects, err := GetConditionalObjectsOrderBy(dbConnection, ProductionOrderRecordTrailTable, conditionString, orderByCondition)
	if err != nil {
