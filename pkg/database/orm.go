...
func CreateRecordTrail(database *gorm.DB, table string, recordTrail RecordTrail) (error, int) {
	err := database.Table(table).Debug().Create(&recordTrail).Error
	if err != nil {
		return err, -1
	} else {
		return nil, recordTrail.Id
	}
}
