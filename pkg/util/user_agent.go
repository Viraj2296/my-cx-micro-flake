...
func GetUserAgentHash(r *http.Request) (error, string, string) {
	zUserIpPort := ReadUserIP(r)

	zUserAgentString := r.Header.Get("User-Agent")
	_userAgent := Parse(zUserAgentString)

	zUniqueString := _userAgent.Name + ":" + _userAgent.Version + ":" + _userAgent.OS + ":" + _userAgent.OSVersion + ":" + _userAgent.Device
	var zUserDevice string
	if _userAgent.Mobile {
		zUserDevice = "Mobile"
	}
	if _userAgent.Tablet {
		zUserDevice = "Tablet"

	}
	if _userAgent.Desktop {
		zUserDevice = "Desktop"
	...
	if _userAgent.URL != "" {
		zUserDevice = "un-recognized"
	}

	zUniqueData := []byte(zUniqueString)
	zHash := md5.Sum(zUniqueData)

	zUserInfo := zUserIpPort + ":" + zUserDevice
	return nil, hex.EncodeToString(zHash[:]), zUserInfo
}
