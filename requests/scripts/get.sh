...
token=$(curl --silent -X POST --data @login.json -H "Content-Type: application/json" $login_url | python3 -c "import sys, json; print(json.load(sys.stdin)['token'])")

echo "Authenticated [" $token "]"

curl -X GET -H "Authorization: Bearer $token" $2
